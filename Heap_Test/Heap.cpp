...
Heap::Heap()
{
    elements.push_back(-1);// This represntes the first in the array The stop point
}
/*
Heap::~Heap()
{
    cout  << "Head is not needed as no pointrs" << endl;
}
** we do not need a destructor. 
*/
void Heap:: insert(int e)// E just represents Key 
{
...
void Heap::fix_Heap()
{
    ...
    while (flag)
    {
        ...
        if(childIndex <= curIdx) // We want the child index if it smaller on the left 
        {
            ...
            int child = getLeftChildValue(index);
...
int Heap::getLeftChildValue(int index){
    return elements[2 * index];
}
int Heap::getRightChildValue(int index){
    return elements[2 * index + 1];
}
int Heap::getParentValue(int index){
